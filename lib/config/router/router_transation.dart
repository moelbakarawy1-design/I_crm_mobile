...
class RouterTransitions {
  ...
  static PageRouteBuilder buildFromBottom(Widget widget) {
    return PageRouteBuilder(
      pageBuilder: (context, animation, secondaryAnimation) => widget,
      transitionsBuilder: (context, animation, secondaryAnimation, child) {
        const begin = Offset(0.0, 1.0); // starts from bottom
        const end = Offset.zero;
        const curve = Curves.easeInOutCubic; // smooth & natural feel
        ...
        return SlideTransition(
          ...
          child: FadeTransition(
            opacity: animation,
            child: child, // optional fade for smoothness
          ),
        );
      },
  ...
  /// Existing vertical (can keep if you want faster slide)
